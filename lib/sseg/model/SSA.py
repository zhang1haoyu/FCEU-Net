...
class Head(nn.Module):
    def __init__(self, num, img_size):
        ...
        self.position_embeddings = nn.Parameter(torch.zeros(1, (img_size // 4) ** 2, 64))
...
class ShuntedTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            if i == 0:
                patch_embed = Head(num_conv, img_size)
            else:
        ...
        '''self.EVC1 = EVCBlock(64,64)
        self.EVC2 = EVCBlock(128,128)
        self.EVC3 = EVCBlock(256,256)
        self.EVC4 = EVCBlock(512,512)'''
    ...
    def forward_features(self, x):
        outs = []
        '''e1, e2, e3, e4 = None, None, None, None  # Initialize variables to store stage outputs'''
        B = x.shape[0]
    ...
    def forward(self, x):
        '''e1, e2, e3, e4 = self.forward_features(x)
...
@register_model
def shunted_t(pretrained=None, img_size=224, **kwargs):
    model = ShuntedTransformer(
        img_size=img_size, patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16],
...
@register_model
def shunted_s(pretrained=None, img_size=224, **kwargs):
    model = ShuntedTransformer(
        img_size=img_size, patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16],
        mlp_ratios=[8, 8, 4, 4], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
...
@register_model
def shunted_b(pretrained=None, img_size=224, **kwargs):
    model = ShuntedTransformer(
        img_size=img_size, patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16],
        mlp_ratios=[8, 8, 4, 4], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
...
if __name__ == "__main__":
    model = shunted_b(img_size=256, pretrained="")
    e1, e2, e3, e4 = model(torch.randn(1, 3, 256, 256))
