...
from lib.sseg.model.SSA import shunted_s, DropPath
...
class HST_UNet_MSCAN(nn.Module):
    def __init__(self, img_size=256, pretrained='', num_classes=1):
        super(HST_UNet_MSCAN, self).__init__()
        ...
        self.attn4 = nn.ModuleList([Block(256) for i in range(4)])

        self.decoder3 = UpSampleBlock(in_channels=256, out_channels=128)
        self.attn3 = nn.ModuleList([Block(128) for i in range(2)])

        self.decoder2 = UpSampleBlock(in_channels=128, out_channels=64)
        self.attn2 = nn.ModuleList([Block(64) for i in range(2)])
...
if __name__ == "__main__":
    ...
    model = HST_UNet_MSCAN(img_size=256)
