...
def parse_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='hst_unet_wf', type=str)  #fceu-net
    ...
    parser.add_argument('--root', default=r'./Potsdam', type=str,
                        help='dataset path')
...
def main():
    ...
    if not args.resume:
        ...
        while os.path.exists(
                args.log_path + args.model + "_Potsdam_ver_" + str(count)):
            count += 1
        args.log_path = args.log_path + args.model + "_Potsdam_ver_" + str(count)
    ...
    # prepare data
    train_dataset = Potsdam(root=args.root,
    ...
    # criterion
    criterion = CriterionSet(loss='EdgeLoss')
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        IoU, IoU_Score, F1_score = MultiClass.test(model=model, dataloader=testloader, criterion=criterion,
        ...
        if IoU_Score > Best_MIoU and F1_score > Best_F1:
            ...
            torch.save({
                ...
                'MIoU': Best_MIoU,
                'F1': Best_F1
            }, os.path.join(args.log_path, f"Best_{epoch}.pth"))
