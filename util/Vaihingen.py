    ...
    if ID not in test and ID not in train:
        ...
        for i in range(0, h, stride):
            for j in range(0, w, stride):
                ...
                if ID in test:
                    image.save(root + f'/Vaihingen/test/img/{i}_{j}_{ID}.tif')
                    crop_mask.save(root + f'/Vaihingen/test/mask/{i}_{j}_{ID}.tif')
                else:
                    image.save(root + f'/Vaihingen/train/img/{i}_{j}_{ID}.tif')
                    crop_mask.save(root + f'/Vaihingen/train/mask/{i}_{j}_{ID}.tif')
...
train_files = os.listdir(r'D:\Vaihingen\Vaihingen\train\img')
test_files = os.listdir(r'D:\Vaihingen\Vaihingen\test\img')

train_txt = open(os.path.join(root, 'Vaihingen','train', 'train.txt'), 'w')
for i, file in enumerate(tqdm(train_files)):
    train_txt.write(file + '\n')

test_txt = open(os.path.join(root, 'Vaihingen','test', 'test.txt'), 'w')
for i, file in enumerate(tqdm(test_files)):
    test_txt.write(file + '\n')

print(mean_std(read_root(r'D:\Vaihingen\Vaihingen\train\img'), h=256, w=256))
